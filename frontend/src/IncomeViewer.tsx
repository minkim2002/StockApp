...
export default function IncomeViewer({ symbol, trigger, setLoading, selectedIndex }: IncomeViewerProps) {
    const [data, setData] = useState<{ date: string; netIncome: number }[]>([]);
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            console.error('Failed to fetch net income:', err);
        } finally {
            setLoading(false);
        }
    };
            ...
            <CardContent className="p-4 space-y-4">
                ...
                {selected ? (
                    <div className="text-center space-y-1">
                        <p className="text-4xl font-bold text-blue-600">${formatIncome(selected.netIncome)}</p>
